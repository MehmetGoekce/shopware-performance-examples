...
@test "php-fpm-pool.conf misst RSS ohne den Master-Prozess" {
    ...
    # als Option.
    run grep -qe '-o rss -C php-fpm8.3' "$CONFIG/php-fpm-pool.conf"
    [ "$status" -ne 0 ]
    run grep -qe '-o rss,args -C php-fpm8.3' "$CONFIG/php-fpm-pool.conf"
    [ "$status" -eq 0 ]
    run grep -q "grep 'pool'" "$CONFIG/php-fpm-pool.conf"
    [ "$status" -eq 0 ]
}

@test "php-fpm-pool.conf verweist auf die robuste Fassung im Companion" {
    ...
    run grep -q 'chapters/09-php-performance/scripts/php-fpm-memory.sh' \
