...
INPUT_ERRORS=$(jq -r '
    ...
    if type != "object" then "Die Eingabe muss ein JSON-Objekt sein, nicht \(type)"
        ...
        (if has("okrs") then
            (list(["okrs"]) // (.okrs | to_entries[] | .key as $i
                       ...
                       (if (.value.key_results | type) != "array" or (.value.key_results | length) == 0
                        then "okrs.\($i).key_results muss eine nicht leere Liste sein"
                            ...
                            | if type != "object" then "okrs.\($i).key_results.\($k) muss ein Objekt sein"
                                ...
                                then "okrs.\($i).key_results.\($k).score muss eine Zahl von 0 bis 1 sein"
                              else empty end
                        end)
                  end))
        else empty end),
        (if has("incidents") then
            (obj(["incidents"]) // (
                ((["p0", "p1", "p2"][]) as $p | nonneg(["incidents", $p])),
                (if .incidents | has("postmortems_done") then nonneg(["incidents", "postmortems_done"]) else empty end),
                ...
                (if .incidents | has("root_causes") then
                    (list(["incidents", "root_causes"]) // (.incidents.root_causes | to_entries[] | .key as $k | .value
                        | if type != "object" or (.cause | type) != "string" or (.count | type) != "number"
                          then "incidents.root_causes.\($k) braucht cause (Text) und count (Zahl)" else empty end))
                 else empty end)))
        else empty end),
...
def objective_score: [.key_results[].score] | add / length | r2;
def okr_total: [.okrs[] | ([.key_results[].score] | add / length)] | add / length | r2;
def cwv_improvement($m): change(.cwv[$m].start; .cwv[$m].end);
def incident_total: .incidents.p0 + .incidents.p1 + .incidents.p2;
def resolved_hours: [.tech_debt.resolved[]?.hours] | add // 0;
def cur: .budget.currency // "CHF";
...
(if has("cwv") then
    ...
    (. as $r | [["LCP", "lcp_ms", "ms"], ["INP", "inp_ms", "ms"], ["CLS", "cls", ""]][]
        ...
        | "| \($lbl) (p75) | \($v.start)\($unit) | \($v.end)\($unit) | **\($r | cwv_improvement($m) | fixed(1))%** \(arrow($v.start; $v.end)) |"),
    "",
    "**Bewertung**:",
    ([(if .cwv.lcp_ms.end > 2500 then "LCP (\(.cwv.lcp_ms.end)ms > 2500ms)" else empty end),
      (if .cwv.inp_ms.end > 200 then "INP (\(.cwv.inp_ms.end)ms > 200ms)" else empty end),
      (if .cwv.cls.end > 0.1 then "CLS (\(.cwv.cls.end) > 0.1)" else empty end)]
...
(if has("okrs") then
    (.okrs | to_entries[] |
        "### Objective \(.key + 1): \(.value.objective)",
        "",
        "| Key Result | Baseline | Target | Erreicht | Score |",
        "|------------|----------|--------|----------|-------|",
        (.value.key_results[] | "| \(.title) | \(.baseline // "–") | \(.target // "–") | \(.achieved // "–") | **\(.score | fixed(2))** |"),
        "",
    ...
    "|-----------|-------|--------|",
    (.okrs[] | "| \(.objective) | \(objective_score | fixed(2)) | \(objective_score | level) |"),
...
(if has("incidents") then
    ...
    (if (.incidents | has("postmortems_done")) or (.incidents | has("mttr_hours")) then
        ...
        (if .incidents | has("postmortems_done") then
            "- Abgeschlossen: \(.incidents.postmortems_done)/\(incident_total)"
            + (if incident_total > 0 then " (\(.incidents.postmortems_done * 100 / incident_total | fixed(0))%)" else "" end)
         else empty end),
        (if .incidents | has("mttr_hours") then "- MTTR (Mean Time to Recovery): \(.incidents.mttr_hours) Stunden" else empty end),
        ""
     else empty end),
    (if (.incidents.root_causes // []) | length > 0 then
        ...
        (.incidents.root_causes | to_entries[]
            | "\(.key + 1). \(.value.cause) (\(.value.count) Incident\(if .value.count == 1 then "" else "s" end))"),
        ""
     else empty end)
...
(if has("tech_debt") then
    ...
    "|--------|---------------|--------------|-------|",
    "| Backlog Items | \($t.start.items) | \($t.end.items) | \(arrow($t.start.items; $t.end.items)) \(change($t.start.items; $t.end.items) | if . == null then "–" else fabs | fixed(0) + "%" end) |",
    "| Geschätzte Stunden | \($t.start.hours)h | \($t.end.hours)h | \(arrow($t.start.hours; $t.end.hours)) \(change($t.start.hours; $t.end.hours) | if . == null then "–" else fabs | fixed(0) + "%" end) |",
    "| Tech Debt Score (Severity-Punkte, Kapitel 15) | \($t.start.score) | \($t.end.score) | \(arrow($t.start.score; $t.end.score)) \($t.end.score | debt_health) (< 200 gesund) |",
    "",
    (if ($t.resolved // []) | length > 0 then
        "### Resolved Tech Debt",
        "",
        ($t.resolved[] | "- \(.title) (\(.hours)h)"),
        "",
...
(if has("budget") then
    ...
    "|-----------|---------|------------|---------|",
    ($b.categories[] | "| \(.category) | \($c) \(.planned | thousands) | \($c) \(.spent | thousands) | \((.spent - .planned) * 100 / .planned | signed(0))% |"),
...
jq -r "${JQ_DEFS}"'
    ...
    "  - Incidents: " + (if has("incidents") then "\(.incidents.p0) P0, \(.incidents.p1) P1" else "keine Daten" end),
    "  - Tech Debt: " + (if has("tech_debt") then "\(resolved_hours)h abgebaut, Score \(.tech_debt.end.score) (\(.tech_debt.end.score | debt_health))" else "keine Daten" end)' \
    "${INPUT_FILE}"
echo ""
