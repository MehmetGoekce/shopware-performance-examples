...
if [[ -n "${SHOP_PATH}" ]]; then
    ...
    # Wie bin/console sie sieht; setzt der Webserver eigene Werte
    # (FPM env[], Apache SetEnv), gelten im Web diese.
    for v in REDIS_SESSION_URL REDIS_URL; do
        dotenv_get "$v"
        [[ -n "${DOTENV_SOURCE}" ]] && echo "   ${v}=${DOTENV_VALUE} (${DOTENV_SOURCE}) $(dotenv_note)"
    done
fi
